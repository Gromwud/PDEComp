...
try:
    ...
    def cut_bound(value, percent=0.02, test=False):
        return value
...
def _is_pdecomp_terms(terms_token):
    return all(
        len(tokens) == 1 and tokens[0].name.startswith("theta_")
        for tokens in terms_token
    )
...
class STRidge(object):
    ...
    def evaluate_terms(self,u,x, test=False, execute_function = unsafe_execute):
        ...
        if _is_pdecomp_terms(self.terms_token):
            omit_list, err_list = [], []
        else:
