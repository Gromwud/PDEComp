...
def register_derivative(values, order, derivative):
    DERIVATIVE_CACHE[(id(values), order)] = np.asarray(derivative, dtype=float)


def cached_derivative(values, order):
    return DERIVATIVE_CACHE.get((id(values), order))
