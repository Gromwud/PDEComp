...
def _tensor2np(value):
    return value.cpu().detach().numpy() if _is_torch_tensor(value) else value


def _feature_token(name, values):
    values = np.asarray(values, dtype=float).reshape(-1)
    return Token(lambda values=values: values, name=name, arity=0, complexity=1)


def _build_pdecomp_library(dataset):
    library_data = get_pdecomp_library(dataset)
    if not library_data:
        return None

    tokens = [Token(np.add, "add", arity=2, complexity=1)]
    tokens.extend(
        _feature_token(f"theta_{index:03d}", library_data["features"][:, index])
        for index in range(library_data["features"].shape[1])
    )
...
class PDETask(HierarchicalTask):
    ...
    def __init__(self, function_set, dataset, metric="residual",
        ...
        pdecomp_library = _build_pdecomp_library(dataset)
        if pdecomp_library is not None:
            self.library = pdecomp_library
        else:
