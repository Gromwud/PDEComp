...
from dso.task.pde import utils_v1
...
PDECOMP_DATASETS = {
    "burgers_data.mat": "add,mul,u1,diff,u1,x1,diff2,u1,x1",
    "burgers_sln_100_data.csv": "mul,u1,diff,u1,x1",
    "ac_data.npy": "add,n3,u1,u1",
    "kdv_data.mat": "add,mul,u1,diff,u1,x1,diff3,u1,x1",
    "kdv_periodic_data.npy": "add,add,mul,u1,diff,u1,x1,diff3,u1,x1,mul,sin,x1,cos,x2",
    "wave_data.csv": "diff2,u1,x1",
    "pde_divide_data.npy": "add,div,diff,u1,x1,x1,diff2,u1,x1",
    "pde_compound_data.npy": "add,mul,u1,diff2,u1,x1,mul,diff,u1,x1,diff,u1,x1",
    "ks_data.mat": "add,mul,u1,diff,u1,x1,add,diff2,u1,x1,diff4,u1,x1",
}
PDECOMP_DATA_REGISTRY = {}
PDECOMP_LIBRARY_REGISTRY = {}


def set_pdecomp_data(dataset, data, x, y, z, t):
    PDECOMP_DATA_REGISTRY[dataset] = (data, x, y, z, t)


def clear_pdecomp_data(dataset=None):
    if dataset is None:
        PDECOMP_DATA_REGISTRY.clear()
        PDECOMP_LIBRARY_REGISTRY.clear()
    else:
        PDECOMP_DATA_REGISTRY.pop(dataset, None)
        PDECOMP_LIBRARY_REGISTRY.pop(dataset, None)


def get_pdecomp_library(dataset):
    return PDECOMP_LIBRARY_REGISTRY.get(dataset)


def load_pdecomp_data(dataset):
    from data.config import sindy_params
    from utils.derivatives import compute_derivative_bundle, get_derivative
    from utils.dataloader import load_data as load_pdecomp_file
    ...
    if dataset in PDECOMP_DATA_REGISTRY:
        data, x, y, z, t = PDECOMP_DATA_REGISTRY[dataset]
    else:
        data, x, y, z, t = load_pdecomp_file(dataset)

    values = np.asarray(data[0] if isinstance(data, list) else data, dtype=float)
    if values.ndim != 2:
        raise ValueError(f"DISCOVER PDEComp mode supports only scalar 1D PDE data, got {dataset}")
    ...
    bundle = compute_derivative_bundle(
        ...
        t=t,
        variable_names=["u"],
        max_orders=configured_max_deriv_order(values.shape, sindy_params[dataset]),
    )
    params = sindy_params[dataset]
    target = params.get("targets", default_targets(["u"]))[0]
    crop_slices = build_crop_slices(values.shape, params.get("crop", 0))
    ...
    PDECOMP_LIBRARY_REGISTRY[dataset] = {
    ...
    utils_v1.clear_derivative_cache()
    for order in range(1, 5):
        utils_v1.register_derivative(
            u,
            order,
            get_derivative(bundle, "u", "x", order).T,
        )
...
def load_data(dataset,noise_level=0, data_amount = 1, training=False,cut_ratio =0.03):
    ...
    """
    if dataset in PDECOMP_DATASETS:
        return load_pdecomp_data(dataset)
