...
try:
    from dso.task.pde.utils_subgrid import ddx, ddy, laplacian, adv, ddx_t, ddy_t, laplacian_t, adv_t
except ImportError:
    force_ops = []
else:
    force_ops = [
...
def add_torch_tokens(function_set, protected):
    if torch is None:
        raise ImportError("PyTorch is required for DISCOVER torch/PINN tokens.")
